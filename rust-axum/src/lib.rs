...
use tower_http::{
  ...
  propagate_header::PropagateHeaderLayer, request_id::SetRequestIdLayer,
  trace::TraceLayer,
};
...
fn user_routes() -> Router {
  Router::new()
    .route(
      ...
      //.layer(HashingMiddleware::hash_user_layer()),
    )
    .route(
      "/user",
      post(user_handlers::save_user)
        // .layer(HashingMiddleware::hash_user_layer()),
    )
    ...
    .route(
      "/user/search",
      post(user_handlers::search_users)
        // .layer(HashingMiddleware::hash_users_layer()),
    )
...
pub fn build_app(
  persist: Arc<dyn UserPersistence>,
